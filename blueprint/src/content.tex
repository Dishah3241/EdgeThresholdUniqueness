...
% g(d), with the geometry this adds to their proof.
%
...
so at $b = m(d)$ each branch closes with the geometry of
Section~\ref{sec:equality-geometry}, which is added to their proof. Each lemma below states
...
\begin{theorem}[Uniqueness for $d \ge 6$]
  ...
  Section~\ref{sec:equality-budget}. Uniqueness at $d = 5$ is Chaffee and Noble's (2017),
  and is not formalized here; a review of this argument (A37) indicates that the same induction
  would also close at $d = 5$, but that remark is not kernel-checked. At $d = 4$ the statement
  fails: $K_{3,3,1}$ is a second extremal graph.
\end{theorem}
