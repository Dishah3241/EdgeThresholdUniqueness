# EdgeThresholdUniqueness working rules
...
- Search pinned Mathlib before introducing a definition or a substantial proof. Reuse its
  ...
  dependency in place.
- No declaration under `EdgeThresholdUniqueness/` may use `sorry`, directly or transitively. The only admitted
...
module that directly provides each name. `lake build` is authoritative because the `EdgeThresholdUniqueness.*`
...
Top-level files in `EdgeThresholdUniqueness/Standalone/Mathlib/` are claims meant to be read by a mathematician
as single files. `InlineEdgeThresholdUniqueness.lean` is the Palomar statement source: **`Challenge.lean` is
...
- Every closed proposition carries a `.witness`; every definition carries a `.separating`.
  ...
  `InlineEdgeThresholdUniqueness.lean` for what each must actually accomplish. `lake exe fidelity` checks
