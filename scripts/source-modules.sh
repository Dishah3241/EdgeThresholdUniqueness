...
# Shared, fail-closed discovery of the Lean modules under EdgeThresholdUniqueness/.
#
...
#
# The lakefile glob `EdgeThresholdUniqueness.*` is authoritative for what is built; this function is the
...
primal_source_modules() {
  ...
  local module_path_re="^EdgeThresholdUniqueness(/[A-Za-z_][A-Za-z0-9_']*)+\.lean$"
  local file module
  ...
  find EdgeThresholdUniqueness -type f -name '*.lean' -print0 | LC_ALL=C sort -z > "$files_out"
  find EdgeThresholdUniqueness -type l -print0 | LC_ALL=C sort -z > "$symlinks_out"
  ...
  if [ ! -s "$files_out" ]; then
    echo 'source-modules: found no EdgeThresholdUniqueness source files; the audit is miswired.' >&2
    return 1
  fi
